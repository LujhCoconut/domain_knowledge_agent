...
| Hybrid Copy 混合页面 Checkpoint | page fault reduction, speculative copy-on-write, hot page migration, DRAM/NVM tiering, active page list | TreeSLS(SOSP'23) |
| 版本化 Checkpoint 一致性 | global version number, per-page versioning, at-most-one backup, NVM runtime page as backup | TreeSLS(SOSP'23) |
...
## Hybrid Copy 混合页面 Checkpoint (TreeSLS)

### 核心问题
高频 checkpoint（1ms 间隔）下，页面拷贝带来的运行时开销成为主要性能瓶颈。TreeSLS 的评估表明，在 1ms checkpoint 间隔下，大部分运行时开销来自 page fault 处理和 page fault handler 中的 page copy。四种传统页面 checkpoint 方法各有缺陷：stop-and-copy（STW 时间过长）、speculative stop-and-copy（重复修改导致重复拷贝浪费）、copy-on-write（将开销从 checkpoint 推迟到运行时——page fault + lock contention）、speculative copy-on-write（误判浪费 CPU）。需要在保证 checkpoint 正确性的前提下，将 page copy 从 critical path 移出。

### 关键洞察

1. **"Hot page → DRAM migration → stop-and-copy; Cold page → NVM stay → copy-on-write"**：利用大多数应用的访问局部性——少数 hot page 被几乎每个 checkpoint 间隔都修改。将这些 hot page 迁移到 DRAM（更快访问+方便 stop-and-copy），其余 cold page 留在 NVM 用 CoW。Hybrid copy 将 page fault 开销降低了 **up to 49%**（Memcached 场景：395 cached pages, 46% page fault elimination）。
- 来源：TreeSLS(SOSP'23)

2. **"Dual-function active page list = hotness tracker + migration work queue"**：每个 page fault → 增加 page 的 hotness 值 → 超过阈值则追加到 active page list。在 STW checkpoint 期间（步骤❸），所有 non-leader core 遍历 active page list 的子列表：(a) dirty DRAM page → stop-and-copy 完成备份；(b) 新追加的 page → NVM-to-DRAM migration；(c) 长时间未访问的 page → DRAM-to-NVM migration + 从 list 移除 + hotness 清零。一份数据结构同时服务 speculative copy 和 page migration。
- 来源：TreeSLS(SOSP'23)

3. **"Idle cores during STW checkpoint = free compute for speculative page copy"**：STW 期间只有 leader core 执行 capability tree 的 checkpoint（~25-100µs），其他 core 被 IPI 中断后进入 idle 等待。TreeSLS 让这些 idle core 在此期间遍历 active page list 做 speculative page copy——不增加额外的 CPU 开销（因为 idle core 本来就在等）。这是一种**"利用系统级 idle time 做投机优化"**的设计模式。
- 来源：TreeSLS(SOSP'23)

4. **"Checkpointed Page Pair (CPP) = 两个备份指针互换角色支持 DRAM/NVM 双向迁移"**：原版 CoW 只需 1 个 backup pointer（NVM runtime page 天然作为另一备份）。Hybrid copy 中 DRAM-cached page 需要 2 个 NVM backup（一个存 in-flight checkpoint，一个存 consistent checkpoint）→ CPP 结构扩展为两个指针。迁移规则保证恢复时：如果 backup version == global version → 用此 backup；否则如果第二 backup version == 0 → 用第二 backup；否则用 version 更高的 backup。
- 来源：TreeSLS(SOSP'23)

### 实践启发
- **"区分 hot/cold + 差异化策略" 是 tiered memory 管理的通用模式**：TreeSLS 的 hybrid copy 本质是对 page 按 hotness 分类 → hot page 获得 eager copy + fast medium (DRAM)，cold page 获得 lazy copy (CoW) + slow medium (NVM)。这与 PACT 的 "hotness ≠ criticality" 互补——PACT 解决"哪些页应该放在 fast tier"，TreeSLS 解决"放在 fast tier 的页如何 checkpoint"。两者可以叠加。
- **"系统级 idle time 是免费的计算资源"**：TreeSLS 利用 STW 期间 idle core 做 speculative copy → 不增加额外 CPU 开销。可推广到任何"stop-the-world + 多核"的同步点——GC pause、lock hold time、batch processing 间隙——如果有多余的 core 在等待，让它们做投机优化不增加 critical path 延迟
- **"Hotness tracking + migration = active page list 的双重功能"**：一个数据结构同时服务 hotness 判断和 migration 调度——不需要单独的 profiling 子系统。类似 Linux LRU 的 active/inactive list 同时服务 reclamation 和 promotion——设计高性能内存管理时，让数据结构承担多重职责是减少 metadata overhead 的有效策略

---

## 版本化 Checkpoint 一致性 (TreeSLS)

### 核心问题
在高频 checkpoint 场景下，如何以最小内存开销维护 checkpoint 之间的一致性？传统方案维护两份 persistent backup（runtime + 两个 backup = 3× 内存开销）或使用 append-only log（GC 开销）。NVM 使 runtime page 本身在断电后不丢失 → 是否可以利用这一特性减少 backup 的数量？

### 关键洞察

1. **"NVM runtime page = 天然的一个 backup——at most 1 extra backup per page"**：传统 DRAM-based checkpoint 必须维护两份 backup（一份 consistent checkpoint + 正在构建的 in-flight checkpoint），因为 runtime page 在 DRAM 上断电即丢。TreeSLS 的 NVM runtime page 断电不丢 → 它天然可以作为 checkpoint 的组成部分 → 每个 page 最多只需 1 个 additional backup。对于非 PMO 对象（Cap Group, Thread, IPC 等），仍维护 2 个 backup（因为它们是完整拷贝而非 CoW）。
- 来源：TreeSLS(SOSP'23)

2. **"Global version number + per-page version = O(1) 恢复决策"**：恢复时只需比较 backup version 与 global version：相等 → 用 backup（runtime page 已被修改）；小于 → 用 runtime page（自上次 checkpoint 未修改）；NULL → 用 runtime page（从未被修改）。**不需要扫描或日志重放——O(1) per page**。
- 来源：TreeSLS(SOSP'23)

3. **"Checkpoint 的正确性 ≠ 每个 page 都有完整的 backup"**：TreeSLS 的 checkpoint 一旦 radix tree 被拷贝到 backup → checkpoint 就生效了（因为 runtime page 在 NVM 上不会丢）。后续的 page copy（CoW 触发）只是"补全"备份，不延迟 checkpoint 的可用性。这与传统 DRAM SLS 必须等所有 dirty page flush 到 storage 才能宣布 checkpoint complete 有本质区别。
- 来源：TreeSLS(SOSP'23)

### 实践启发
- **"利用硬件持久性简化软件协议"**：NVM 的持久性使 TreeSLS 的版本协议从"3 copies"简化为"1 or 2 copies"。类比 eADR 使 CPU cache 数据可在断电时 flush 到 NVM → 消除了 cache flush 指令的必要性。任何持久化协议都应该识别底层硬件提供的保证，避免软件冗余维护硬件已有的属性。
- **"版本号 + 指针 = 最简快照一致性"**：不需要 WAL、不需要 undo/redo log、不需要 reference counting——仅 global 版本 + per-page 版本 + 1-2 个 backup pointer → 可以处理任意时刻的崩溃恢复。灵感可以应用于用户态数据结构快照（如 in-memory database 的 snapshot isolation）——如果底层存储支持 CoW，快照可以简化为版本号翻转
