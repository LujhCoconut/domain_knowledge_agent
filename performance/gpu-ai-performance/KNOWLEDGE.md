...
## Agent-Native MoE 训练系统设计 (PithTrain)

### 核心问题

生产级 MoE 训练框架（Megatron-LM 149K 行、DeepSpeed 167K 行）经过多年工程积累实现了极高训练吞吐，但其设计决策——插件系统、注册表间接引用、大量 C++/CUDA 扩展——对 AI coding agent 理解、操作和扩展框架造成了隐性成本。传统评估只看吞吐，完全忽略 agent 维度。挑战在于：能否设计一个既匹配生产吞吐、又对 agent 友好的训练框架？

### 关键洞察

1. **"11K 行 Python-native 代码库可匹敌 149K 行生产框架的吞吐"**：PithTrain 以 ~11K 行纯 Python（仅算子用 Python DSL 调用 Triton/ThunderKittens/DeepGEMM/FlashAttn）实现端到端 MoE 训练，在 5 个配置中 4 个超越 Megatron-LM 吞吐。证明 compact 代码库不以牺牲性能为代价。

2. **"DualPipeV：DeepSeek-V3 DualPipe 的开源实现 + 实际 compute-comm overlap"**：DeepSeek 开源版本仅提供最小 pipeline 编排骨架。PithTrain 实现五阶段分解（在 EP 通信边界拆分），EP all-to-all 跑在独立通信流上，forward 和 backward 跨 micro-batch 重叠。配合 wgrad delay 进一步减少 bubble。

3. **"torch.compile(fullgraph=True) 严格模式优于宽松模式"**：宽松模式（允许 graph break）在编译时静默降级 speedup 而不报错——开发者无感知。严格模式在编译时拒绝 graph break，迫使开发者发现问题并修复。MoE forward/backward 因 per-expert 输入 shape 数据依赖而排除在外。

4. **"FP8 weight cache 跨 micro-batch 避免重量化"**：同一 micro-batch 组内 weights 不变，缓存量化结果避免每次重新 FP8 quantize。EP dispatch 去重降低 all-to-all 通信量。这些优化虽非新颖技术，但在 compact codebase 中的实现证明了"系统性应用已知优化"的价值。

- 来源：PithTrain(arXiv'26)

### 实践启发

- **"torch.compile fullgraph=True 是比宽松模式更好的默认值"**：宽松模式隐藏了 graph break → 开发者以为在用 compiled mode，实际可能是 partial compilation。严格模式将 graph break 从不透明的性能退化变成显式的编译错误 → 迫使修复
- **"DualPipe 的正确实现 = 在 EP 边界插入通信 + 跨 micro-batch overlap"**：不是微调开源 scaffold，而是从头实现五阶段分解 + 通信流解耦 + forward/backward 交替
- **"Compact ≠ 缺少功能，而是只做必要的事"**：PithTrain 覆盖 PP/DP/CP/EP/FP8/DCP checkpointing——这不是"只做 80%"，而是"做了 100% 的 MoE 训练必要功能，但不做额外覆盖"
- **"FP8 weight cache 适合任何有重复量化开销的 pipeline"**：micro-batch 训练、beam search 推理、KV cache 场景都可能有重复计算的 weight quantization → 缓存策略可直接复用

---

## LLM 抢占式 Iteration-Level 调度 (FastServe)

### 核心问题
LLM serving 系统（vLLM/Orca）使用 FCFS + run-to-completion 处理推理请求。真实工作中输出长度呈重尾分布 → 长请求堵塞短请求 → **queuing delay 占端到端延迟 90%+**。优化执行时间已不够——必须优化排队延迟。LLM 推理的独特信息状态是输出长度不可预知（无法用 SRPT），但输入长度在请求到达时就已知——这是一个 **semi information-agnostic** 的设定，既不是全盲也不是全知。经典 MLFQ 假设全盲且不能处理 LLM 的两阶段执行（prefill 时间长 → 第一轮 quantum 就耗尽 → 被抢占 → 计算浪费）。

### 关键洞察

1. **"Semi information-agnostic 是 LLM serving 调度问题的精确刻画"**：input length 已知（→ 可准确 profile prefill 时间）但 output length 未知（→ 不能预先知道 job size）。这一信息状态是 LLM 推理独有的——FastServe 首次清晰定义了它，并用 skip-join 从中廉价提取价值。

2. **"Skip-join = 用确定性信息替代 ML 预测——更简单、更可靠"**：不是训练模型预测 output length，而是用 profiling 得到精确的 prefill 时间 → 新 job 直接 join 到 `q_i ≥ t_prefill` 的最高优先级队列 → 跳过无意义的降级 → 零预测误差、零 ML 开销。这体现了"确定性 information 比统计 model 更可靠"的工程哲学。

3. **"ENST 联合考虑 promotion 和 higher-priority execution —— 不是简单按优先级排序"**：决定 KV cache swap 顺序时，不能简单说"低优先级=swapout"。Starvation prevention 可能突然提升低优先级 job → ENST 同时考虑了 T_promote（距饥饿提升的时间）和 T_execute（所有高优先级 job 的 quantum 之和/batch_size）→ 选取最小值。这是一个 preemption-aware 的 I/O 调度决策。

4. **"Proactive > Reactive —— 预测 + 流水线 > 响应 + 阻塞"**：不是等 job 需要时再 swap in KV cache，而是预测 ENST → 提前在 GPU 执行期间异步 swap → PCIe 传输与 kernel 执行重叠 → swap time <5% 总延迟（reactively 则是关键路径上的数百 ms）。

5. **"Pipeline parallelism 打破 MLFQ 语义 —— pending state 最高优先级规则修复"**：PP 各 stage 独立调度 → job A 在 stage 1 执行时 stage 2 可能调度了 job B → 传统 MLFQ "同一 job 持续运行到超时" 的语义被打破。FastServe 保持 job 在队列中、调度 pending 中最高优先级的 job → 近似保持 MLFQ 语义且不牺牲 PP 的吞吐优势。

- 来源：FastServe(NSDI'26)

### 实践启发
- **"抢占粒度 = 应用的自然原子边界"**：LLM 每生成一个 token 是自然抢占点 → 零重算开销。**可推广模式**：不强制中断（OS-style preemption），而是利用应用自己的 natural yield point（transaction commit、request completion、frame boundary）做抢占 → 消除状态保存/恢复的 overhead。
