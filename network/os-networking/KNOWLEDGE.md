...
| 流内学习拥塞控制 | intra-flow learning, Hoeffding Adaptive Tree, online exploration, dynamic model switching, non-parametric streaming model | PolicyCache(NSDI'26) |
...
## 流内学习拥塞控制 (PolicyCache)

### 核心问题
拥塞控制（CC）面临"不可能三角"：启发式方法（CUBIC/BBR）轻量但脆弱，DRL 方法（Aurora/Orca）响应快但泛化差，探索方法（Vivace）适应性强但收敛慢。根本原因在于 **inter-flow learning** 范式：策略在某个网络环境学习后应用到新流——隐含假设"状态→动作映射跨环境一致"，这个假设在真实网络中根本不成立（rate limiter、traffic shaper、ACK aggregation、delayed ACK 等机制在发送端不可见）。

### 关键洞察

1. **"Intra-flow learning 将跨环境泛化问题降维为单流内时序一致性问题"**：与其训练一个万能策略应用到所有流，不如每条流从自己的观测中在线学习自己的策略。假设从"策略在所有网络环境有效"弱化为"策略在当前流当前网络条件下有效"——这一弱化足以消除泛化问题。
2. **"非参模型是 online learning 中被严重低估的选择"**：DRL 依赖 SGD 训练 DNN——需要数千样本和数小时训练，远超多数流的生命周期。Hoeffding Adaptive Tree (HAT) 通过"记忆"观测到的模式实现即时利用，仅需数个 RTT 收敛。本质区别：参数模型学函数形式（inductive），非参模型存实例（transductive）。
3. **"探索/执行双模切换 + 性能分数提供性能下界保证"**：备份探索模式（简化版 Vivace）在模型未训练时保证不低于纯探索方法的性能；模型执行模式在策略学会后提供快速响应。性能分数 p 作为切换判据——这是一个可复用的控制架构模式。
4. **"微探索统一样本收集，避免执行模式的鸡生蛋问题"**：模型执行时如何获取 ground-truth label？PolicyCache 在模型输出上叠加小幅双向扰动（与备份探索相同的 ±α），使得即使在执行模式下也能持续生成训练样本和更新性能分数。两种模式的样本收集逻辑完全统一，保证收敛证明的简洁性。
5. **"State-action alignment 解决观测延迟导致的标签噪声"**：拥塞控制中动作效果需至少 1 RTT 后才可观测。直接配对"最新状态 + 当前动作"会引入标签噪声。PolicyCache 维护滑动窗口缓存历史状态+interval ID 索引 → 延迟到达的 utility 反馈精准对齐到因果状态。这一机制可推广到任何存在观测延迟的在线学习系统。
6. **"Utility 单调性驱动去中心化公平收敛"**：Utility 函数中 −b·xᵢ·dRTT/dt 项使高发送速率流增加速率的概率严格低于低速率流 → 各流异步、独立决策 → 自然收敛到公平均衡。不需要集中式协调或显式公平约束——公平性从 utility 设计自然涌现。

- 来源：PolicyCache(NSDI'26)

### 实践启发

- **"Intra-X learning 范式可推广到其他序列决策场景"**：任何需要在线适应、无法预训练通用策略、且每个实例有足够观测样本的问题——自适应视频码率（ABR）、动态资源分配、per-instance cache 驱逐策略、甚至是 per-device 电源管理——都可以考虑"每实例自己学自己的非参模型"。关键是判断"单实例观测是否足够支撑模型收敛"。
- **"用非参模型替换参数模型可大幅降低在线学习门槛"**：HAT 增量更新开销极低（一次树遍历 + 叶节点统计更新 vs 矩阵乘法+反向传播），使得在资源受限设备上做在线学习成为可能。这一洞察可直接应用于移动端/嵌入式的自适应网络协议。
- **"双模架构（backup + learned + score 切换）是鲁棒在线控制系统的通用蓝图"**：backup 保证下界（worst-case performance），learned 追求上界（best-case performance），score 决定切换时机。三个组件职责分明、独立可替换——backup 可以是任何保守策略，learned 可以是任何在线模型。
- **"微探索能以极低成本在线持续获取 ground-truth"**：与其单独运行探索阶段，不如在正常操作中叠加小扰动——成本几乎为零（PolicyCache 中每对探索的总发包数等于两个非探索间隔）。适用于任何需要在线评估模型质量的场景。
- **"公平性可以从 utility 设计自然涌现，无需显式约束"**：PolicyCache 的收敛证明核心是 utility 函数中与自身速率成正比的惩罚项→自然单调性。这一设计原则（在目标函数中嵌入自限制项）可推广到分布式资源分配、负载均衡等场景。
- **"Agent-CC 协同设计与 UEP/UCCL-Tran 共享哲学"**：PolicyCache 的作者（Han Tian 等同组）在 CC 方向有三篇连续工作——Spine(CoNEXT'22) → Astraea(EuroSys'24) → PolicyCache(NSDI'26)，以及 Jury(EuroSys'25) 解决公平性泛化。这些工作构成了一条从"DRL 优化性能"→"DRL 公平性"→"抛弃 DRL 用非参在线学习"的演进线，体现了对"学习在 CC 中如何应用"这一根本问题的持续追问。
