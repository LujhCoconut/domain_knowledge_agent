...
| Single-Tier 单层存储 SLS (NVM) | single-level store, non-volatile memory, byte-addressable persistence, write amplification elimination | TreeSLS(SOSP'23) |
| Capability Tree 全系统状态 Checkpoint | microkernel, capability derivation tree, incremental checkpoint, tree traversal, decentralized state | TreeSLS(SOSP'23) |
...
## Single-Tier 单层存储 SLS (TreeSLS)

### 核心问题
传统 SLS（Single-Level Store）号称"单层存储"，但实际构建在 DRAM + Disk/SSD 两层硬件之上——仅是软件层提供了单层 illusion。这导致两个根本问题：(1) **write amplification**：CPU 按字节操作，但必须按 block 刷到存储，一个字节的修改可能触发整页/整 block 的持久化；(2) **checkpoint 频率天花板**：checkpoint 必须把 dirty data 从 DRAM 拷贝到存储，在存储延迟远高于 DRAM 的硬件约束下，传统 SLS 的 checkpoint 间隔只能达分钟级（Aurora 即使使用 NVMe 也只能 5-7ms 一次）。NVM（如 Intel Optane PM + eADR）将字节寻址、DRAM 级延迟和存储级持久性三者合一，使真正的 single-tier SLS 成为可能——但"把系统放在 NVM 上运行"并不自动等于"全系统持久"，CPU 寄存器和设备寄存器仍然易失，软件 checkpointing 仍然必需且需要重新设计。

### 关键洞察

1. **"NVM 既是 runtime memory 又是 storage——消除 device distinction 就是消除 write amplification"**：传统 SLS 的 write amplification 源于 runtime memory (DRAM) 和 storage (Disk/SSD) 的访问粒度差异（byte vs block）。NVM 使 CPU 可以直接 byte-addressable 地操作持久化数据——runtime page 和 checkpoint page 在同一介质上，无需"从 memory buffer 拷贝到 storage"。增量 checkpoint 的 checkpoint size 甚至小于应用的 runtime memory 消耗（因为未修改的 runtime page 可以直接作为 checkpoint 的一部分）。
- 来源：TreeSLS(SOSP'23)

2. **"~100µs checkpoint 使 1ms 高频 checkpoint 成为可能——checkpoint 不再是 heavyweight 操作"**：传统 SLS 分钟级 checkpoint → 丢失窗口大。TreeSLS 的 1ms checkpoint 将丢失窗口从分钟缩到毫秒 → 外部可见操作的延迟仅需 ~1 个 checkpoint interval → 外同步可以被透明化（延迟网络 I/O 直到下一 checkpoint）。
- 来源：TreeSLS(SOSP'23)

### 实践启发
- **"介质统一 = 消除数据搬迁"**：当计算介质和存储介质是同一物理介质时，checkpoint 不再是"从内存拷贝到磁盘"，而是"在介质内标记版本"。可推广到任何"快照/备份"场景——当底层天然支持 CoW/版本时，checkpoint 简化为元数据操作
- **"延迟降低→频率提升→语义简化"**：当 checkpoint 延迟从分钟降到微秒，高频 checkpoint 使 external synchrony 可以通过"延迟 I/O 到下一次 checkpoint"透明实现，无需应用修改。这是一条通用设计原则：**系统原语的延迟每降一个数量级，上层语义就可能被简化一个层次**

---

## Capability Tree 全系统状态 Checkpoint (TreeSLS)

### 核心问题
全系统持久化需要 checkpoint 整个 OS 的状态——在 monolithic kernel 中这意味着要理解 FD table、dentry cache、inode cache、socket buffer、pipe buffer 等几十种不同的内核数据结构的内部关系和依赖，checkpoint 逻辑极其复杂且脆弱。微内核架构将大部分系统服务（文件系统、网络栈、驱动）推到用户态——从内核角度看它们只是普通进程——checkpoint 逻辑不需要理解这些服务的内部结构。但这消除了大部分复杂度后还有剩余：内核自身的对象（进程、线程、IPC 连接、内存区域、通知对象等）如何高效 checkpoint？

### 关键洞察

1. **"Capability derivation tree = 系统全部状态的天然索引"**：seL4/ChCore 等现代微内核使用 capability-based access control——每个内核对象由一个 capability 引用，所有 capability 组成一棵 derivation tree。TreeSLS 的洞察：**这棵树不是需要额外维护的检查点结构——它就是系统状态的天然表示**。Checkpoint 简化为树的深度优先遍历——对每个 capability 指向的对象按类型执行对应的序列化逻辑（Cap Group→拷贝 capability 数组，Thread→拷贝寄存器上下文，PMO→标记页为 CoW 等），递归完成。对于 monolithic kernel，等价操作需要遍历/proc、遍历每个子系统的内部链表、理解数十种对象间的隐式关系——复杂度高出一个数量级。
- 来源：TreeSLS(SOSP'23)

2. **"Checkpoint 时不需要拷贝内存页——NVM 的持久性使 runtime page 天然就是 checkpoint 的组成部分"**：传统 checkpoint 必须把 dirty page 拷贝到 checkpoint area。TreeSLS 在 NVM 上只需要标记 PMO 的 radix tree 为已 checkpoint——runtime page 本身就在 NVM 上，断电不丢失。后续修改通过 CoW 触发 page copy 到 backup。这使 PMO checkpoint 从 O(dirty_pages) 降为 O(radix_tree_nodes)——**checkpoint size 仅为 runtime 的 55-80%**。
- 来源：TreeSLS(SOSP'23)

3. **"不需要 checkpoint 的数据可以不纳入能力树"**：Page table（可重建）、scheduler 队列（可从 Thread 对象重建）、某些内核 buffer——这些 derived state 被排除在 capability tree 之外，在恢复时重建。这是一种**"checkpoint 最小集合"的思辨**：什么状态是不可重建的（must checkpoint），什么是可从不可重建状态推导的（rebuild on restore）。
- 来源：TreeSLS(SOSP'23)

### 实践启发
- **"利用已有的数据结构的索引功能——不要为 checkpoint/crash-consistency 重新发明索引"**：Capability tree 本身就是 seL4/ChCore 的访问控制结构，TreeSLS 直接复用它作为 checkpoint 结构。**可推广模式**：任何已有维护的全局数据结构（Radix tree、B-tree、hash table）如果已经覆盖了需要持久化的所有对象，就可以直接作为 checkpoint/恢复的遍历入口——不需要单独维护 dirty object list
- **"Checkpoint 最小集合 = 不可重建状态"**：设计 checkpoint 系统时，第一步不是"checkpoint 什么"，而是"什么可以重建"。page table 可重建、scheduler 队列可重建、某些统计数据可重建——把这些排除后剩下的"必须 checkpoint"集合往往出人意料地小
- **"Microkernel architecture → simplified whole-system reasoning"**：这不仅是安全/可靠性的论证——TreeSLS 证明了对全系统状态 checkpoint 而言，微内核的模块化也显著降低了 checkpoint 的复杂度。类似 InfiniDefrag "多一层虚拟化 = 多一个解决碎片的机会"——**微内核的架构优势不仅体现在隔离性，也体现在可观测性/可 checkpoint 性上**
