...
| 微服务数据完整性静态检测 | ER model, relational algebra, SSA, abstract call graph, cross-service data integrity | Aletheia(OSDI'26) |
| Shell 管道静态类型检查 | regular types, streaming shell, type checking, finite-state transducer, polynomial-time, counterexample reporting, byte stream | RT(OSDI'26) |
...
### 核心问题
微服务将数据拆到独立数据库后，跨服务数据的逻辑关系（如"订单必须对应有效用户"）不再有 DB 级 foreign key 保护——全由**应用代码手动维护**。随着微服务规模和复杂度增长，这种手动正确性实际上不可保证。

### 关键洞察

1. **经典形式化可跨领域移植**：ER 模型 + 关系代数（50 年前的发明）被重用于定义跨服务数据完整性的违规语义
2. **SSA + 抽象调用图的组合**：编译器领域的 SSA 追踪每个微服务内部的数据流，跨服务的抽象调用图追踪 RPC/HTTP 链
3. **定义了三种规范的违规类型**：referential integrity, cardinality, 和第三种——覆盖了分布式数据完整性被破坏的主要方式
- 来源：Aletheia(OSDI'26)

### 实践启发
- 微服务的数据完整性是被严重低估的问题：46 个违规在 7 个开源应用中表明不是边缘现象
- 46 这个数字说明：几乎每个非平凡的微服务应用在手动维护跨服务数据关系时都会出错
- ER 模型作为跨领域工具：不仅仅是 DB schema design——也可以用来做分布式程序的正确性分析
...
### 核心问题
Shell 是无类型环境——命令通过 byte stream 通信，类型不匹配只在运行时发现（程序崩溃、静默文件损坏）。数十年的 Shell 编程中，开发者只能通过执行发现组合错误。与 hS（推测执行）和 Incr（增量执行）不同，RT 专注于编译时安全：**在 Shell 程序运行前静态检测管道兼容性错误**。

### 关键洞察

1. **"Regular types = 正则表达式作为类型——适合 shell 的流式本质"**：Shell 命令的 I/O 可以被正则表达式描述（"只有 [a-z]+ 行"、"每行三个 TAB 分隔字段"）→type checking = 语言包含（正则语言包含是多项式复杂度）→低 overhead。错误消息本身是正则表达式→开发者熟悉 Regex 语法。类似 TypeCraft "type annotations on DWARF"——将类型信息嫁接到已有工作流。
2. **"Finite-state transducer 处理复杂命令变换"**：某些命令的 I/O 不能用简单正则捕获（如 tr、sed 的复杂模式）→有限状态变换器捕获更复杂的变换→扩展表达能力。
3. **"Polynomial-time type checking——practically zero cost"**：正则语言包含可在多项式时间内判定→平均 0.02s 检查时间→可以在开发过程中实时反馈→91% 精度检测严重错误。

- 来源：RT(OSDI'26)

### 实践启发
- **"Regular types are the right abstraction for byte-stream processing"**：正则表达式已有数十年历史、开发工具支持完善→选择熟悉的形式给予低 adoption 门槛。类型系统不需要 fancy——需要 practical
- **"与 hS、Incr 形成 Shell 编程安全三部曲"**：hS 加速执行（推测+乱序）、Incr 缓存中间结果、RT 静态检查类型——分别解决并行性、增量性和正确性。三部曲完整覆盖 Shell 编程的不同安全维度
