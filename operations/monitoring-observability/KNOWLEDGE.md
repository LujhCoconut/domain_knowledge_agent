...
| 云原生时序存储模型 | cloud object storage, metadata-data separation, tag dictionary, Patricia trie, CSR bitmap compression, timeseries-tag index, read amplification elimination | CloudTS(FAST'26) |
...
### 核心问题
Prometheus/Cortex 等性能监控时序系统的时间分区文件打包 metadata+data→直接转 cloud object→少量查询也需下载整个 object→严重读放大。十亿级时序中 >80% metadata 是 tags，且 >70% tags 跨时间分区重复存储+重复访问。Parquet 列存可压缩但不解决按时序过滤的读放大；JTS 按时序分 object→tag-based 查询需扫描全部 objects→不可扩展。

### 关键洞察

1. **"metadata-data 分离 + 全局去重 → 按需下载消除读放大"**：全局 TagDict (Patricia Trie + 双向指针 + 时间分区感知) 消除跨 partition tag 重复→metadata 从 "每个 partition 重复存储" 变为 "全局一次+每 partition local tag array"。TSObject 单独存储压缩数据 chunk→查询时仅下载相关 TagDict entries + target TTMapping + target TSObjects→无需下载整个 block。

2. **"CSR 压缩的二维 bitmap 保留双向索引能力"**：TTMapping (row=时序ID, col=tag encoding)→原始 bitmap O(M×N)→TMMC 压缩 (仅存 ind + ptr 两个数组)→O(M+N) 空间，同时保留 row 方向(时序→tags)和 column 方向扫描(tag→时序列表)的双向查询→无需分离正排和倒排索引。

3. **"Tag frequency 分层分组 = 按选择性分流查询"**：低频 tag pair(高选择性)→直接过滤无关时序组；高频 tag pair(共享>90%时序)→不用于过滤但用于 TSObject 并行分组。这是经典信息检索中 inverted index pruning 的思路在时序 metadata 中的应用。

- 来源：CloudTS(FAST'26)

### 实践启发
- **"metadata-data 分离+全局去重 → 云对象存储的通用模式"**：不仅是时序数据——任何需要频繁 metadata 查询但数据量大的场景(日志索引、KV store 元数据、文件系统 inode)都可以用此模式消除读放大
- **"CSR 压缩 = 稀疏 binary matrix 的双向索引最优方案"**：当关联矩阵是稀疏 binary 矩阵时，CSR 的 ind+ptr 同时支持 row 和 column scan→这是比"正向索引+反向索引"更紧凑的设计
- **"标签去重+编码 = 信息检索的经典思路在云存储中的再应用"**：类似数据库的 dictionary encoding——全局字典+局部编码→节省空间+加速查询
