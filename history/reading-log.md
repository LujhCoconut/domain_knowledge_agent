...
| 2026-07-17 | RosenBridge(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-qiu.pdf, XMU/KylinSoft/Huawei/SJTU/NUDT/THU | knowledge-synthesis | operations/cloud-infrastructure/ | virtio-ndp + uBPF 将 bare-metal NDP express I/O (XRP/GDS) 引入虚拟化环境，CPU 用量大幅降低，性能接近裸金属 |
| 2026-07-17 | MlsDisk(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-xu.pdf, SJTU/Ant Group/XMU/THU | knowledge-synthesis | security/os-security/ | 四层分层安全日志替代 MHT in-place 更新，SGX-PFS 替代方案，7.3-21.1× 微基准提升，支持 irreversibility + atomicity 扩展 |
| 2026-07-17 | Jitskit(arXiv'26) | 论文-愿景/系统 | arXiv:2605.24096, UC Berkeley (Ion Stoica 组) | knowledge-synthesis | architecture/agent-native-design/ | JIT 系统合成：三张 spec cards → planner/coder/critic/auditor 迭代生成 KV store，18/18 spec 超越 FASTER/F2/RocksDB/Redis，最高 4.6×，naive Claude Code 被 reward hack 或低 5.4× |
| 2026-07-17 | SkySync(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-zhang-zhihao.pdf, XMU/Alibaba Cloud/SJTU/THU | knowledge-synthesis | performance/storage-filesystem/ | 复用存储层已有 checksum (CRC32C/SHA) 替代 rsync/dsync 重算，delta generation 开销 -89.3%，sync 性能 +1.1-2× |
| 2026-07-17 | ParaSync(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-zhang-zhihao-parasync.pdf, XMU/Alibaba Cloud/SJTU/SYSU/THU | knowledge-synthesis | performance/storage-filesystem/ | 打破 CDC 文件同步的三阶段串行依赖：CRC32C 组合实现 sub-chunk 并行 chunking + 流式匹配消除 all-or-nothing 交换 + 绝对偏移 pipelined 重构，chunking 7.6× 加速，端到端 3.7× |
| 2026-07-17 | DisCoGC(FAST'26) | 论文-系统(工业) | FAST '26, usenix.org/system/files/fast26-bian.pdf, ByteDance/THU | knowledge-synthesis | performance/storage-filesystem/ | ByteDance ByteStore 生产系统：discard+compaction 组合 GC 打破 WA-SA trade-off，WA -25%，TCO -20%（每月数百万美元） |
| 2026-07-17 | PolarStore(FAST'26) | 论文-系统(工业) | FAST '26, usenix.org/system/files/fast26-hu.pdf, Alibaba Cloud | knowledge-synthesis | performance/storage-filesystem/ | 阿里 PolarDB 双层压缩：PolarCSD 硬件 + 软件协同，3.55× 压缩比、60% 存储成本降低、千台服务器 100+ PB 生产部署 |
| 2026-07-17 | ZUFS(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-kim-jungae.pdf, SK hynix/Google/SNU | knowledge-synthesis | performance/storage-filesystem/ | Zoned UFS 跨层优化：动态 SRAM 共享 + 写序保证 + 主动 GC，碎片化下写吞吐 2×、游戏加载 -14%，商用手机验证，已推入 JEDEC + Android 主线 |
| 2026-07-17 | DOGI(FAST'26) | 论文-方法 | FAST '26, usenix.org/system/files/fast26-kim-jeeyun.pdf, POSTECH/DGIST/Virginia Tech | knowledge-synthesis | performance/storage-filesystem/ | NoDaP oracle 确立 WAF 上界 + DOGI 混合启发式/ML 预测 invalidation time + 动态 group 配置，WAF -23.2%、写吞吐 +13.3% vs SOTA |
| 2026-07-17 | MOST(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-tu.pdf, UW-Madison/Google | knowledge-synthesis | performance/storage-filesystem/ | 镜像少量热数据 + tiering 冷数据：结合 mirroring 负载均衡与 tiering 空间效率，在 I/O 密集和动态 workload 下优于纯 tiering/caching/mirroring |
| 2026-07-17 | WSBuffer(FAST'26) | 论文-系统 | FAST '26, usenix.org/system/files/fast26-zhan.pdf, HUST/UT Arlington | knowledge-synthesis | performance/storage-filesystem/ | Scrap buffer 替代 page cache 缓冲所有写入：大块对齐直接发 SSD、小块/非对齐缓存，消除 read-before-write 开销和 page management 瓶颈，吞吐 +3.91× vs EXT4/F2FS/BTRFS/XFS |
| 2026-07-17 | DRBoost(FAST'26) | 论文-方法 | FAST '26, usenix.org/system/files/fast26-niu.pdf, THU | knowledge-synthesis | performance/storage-filesystem/ | MSR code 大 chunk 导致的 degraded read I/O 放大优化：partial-chunk reconstruction + coding layout + fragmentation-free layout，延迟 -1~2 数量级 |
| 2026-07-17 | LESS(FAST'26) | 论文-编码理论 | FAST '26, usenix.org/system/files/fast26-cheng.pdf, CUHK/Shandong U/HUST | knowledge-synthesis | performance/storage-filesystem/ | 分层扩展子条带 (extended sub-stripes) 同时降低修复带宽和 I/O seeks，小 sub-packetization，HDFS 实测修复时间显著降低 |
