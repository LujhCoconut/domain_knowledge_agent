...
### 核心问题
SASOS 的 lightweightness 来自同一地址空间（零页表切换、零 TLB flush、快速安全域切换）。但 POSIX fork 语义建立在多地址空间之上→SASOS 无法运行 50% of 最流行 Debian packages。过往方案在 lightweightness/compatibility/isolation 三角中总牺牲一角：segment-relative addressing (Angel/Mungi) 需大规模 compiler 修改→不现实；OS-as-a-process (Nephele/KylinX/Graphene) 重新引入多 AS→丧失 lightweightness；Junction/OSv 放弃 isolation→不安全。μFork 是首个同时满足三者且 self-contained 的方案。

### 关键洞察

1. **"μprocess = 同一 AS 内进程模拟，不创建新页表"**：父 μprocess fork→在同一虚拟 AS 中找空闲连续区域→复制父内存到新位置给子 μprocess。保持 SASOS 的零 context switch 优势。两个核心挑战：(C1) 子进程所有绝对指针指向父区域→需重定位；(C2) 需强 μprocess 间 + user/kernel 隔离→不能牺牲 lightweightness。PIC 编译使绝大部分内存引用为相对（PC/SP relative）→仅绝对引用需重定位→最小化 fork 开销。

2. **"CHERI capability——一个硬件机制同时解决重定位和隔离"**：128-bit capability（含 bounds、permissions、1-bit unforgeable validity tag in separate tag memory）。(C1) 重定位：16-byte 扫描检查 tag→valid tag 即 capability→精确重定位真指针（整数被安全忽略）→解决 pointer tracking 这个公认 hard problem。(C2) 隔离：capability bounds 在硬件解引用时 enforce→bounds 只能缩小不能扩大(monotonicity)→μprocess 无法逃逸内存区域。类似 InfiniDefrag "GPA 是虚拟的"——硬件能力被重新解释为 OS 构建块。

3. **"CoPA (Copy-on-Pointer-Access) = fork 优化的第三维度"**：三级策略：full copy → CoA (Copy-on-Access，任何访问触发) → CoPA（仅 write 或 child load capability 时触发）。CoPA 通过 CHERI **capability-load fault** 实现——页表额外 permission bit→加载 capability 时硬件精确 trap→普通数据读取不触发→最大化页共享。16-byte 粒度扫描重定位 capability→bound 限制到子 μprocess 区域。Poiner 是特殊数据——不应在仅读数据时复制。

4. **"Sealed capability = trapless syscall"**：传统 syscall 需 costly trap→CHERI sealed capability 允许 μprocess 持有 sealed kernel entry point→调用时硬件验证→直接跳转 kernel→消除 trap 开销（satisfies lightweightness requirement R1）。同时 μprocess 无法直接执行特权指令（capability 无 system permission bit）。

5. **"Parameterized isolation = 三个威胁模型、三种配置"**：(U3) 特权分离(OpenSSH/qmail)→adversarial fault isolation + TOCTTOU 保护；(U2) 并发(Nginx)→non-adversarial fault isolation；(U4) CoW 快照(Redis)→完全禁用隔离。Per-deployment 配置→不付不需要的安全代价。

- 来源：μFork(SOSP'25)

### 实践启发
- **"CHERI 不仅是安全特性——是 OS 设计原语"**：capability tag→指针识别、bounds→intra-AS 隔离、sealed cap→fast syscall、cap-load fault→CoPA。四个 CHERI 特性分别解 SASOS fork 四个子问题。硬件能力的重新解释解锁全新 OS 设计
