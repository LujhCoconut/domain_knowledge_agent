...
| 单地址空间 OS Fork | SASOS, CHERI, capability, μprocess, Copy-on-Pointer-Access, pointer relocation, intra-address-space isolation, unikernel | μFork(SOSP'25) |
...
### 核心问题
单地址空间 OS (SASOS) 有天然 lightweightness 优势——kernel 和应用共享同一地址空间→零页表切换、零 TLB flush。但 POSIX fork 的语义建立在多地址空间之上（子进程需要独立 AS），使 SASOS 无法运行多进程 POSIX 应用。过往方案在 lightweightness / compatibility / isolation 三者之间总是牺牲其一：复制到新 AS（丧失 lightweightness）、不支持完整 POSIX（丧失 compatibility）、或弱隔离（丧失 security）。

### 关键洞察

1. **"μprocess = 同地址空间内的进程模拟，不是创建新 AS"**：在同一 SAS 内将父进程内存复制到不同位置→形成逻辑独立的 μprocess。两个核心挑战：(C1) 子进程所有绝对指针指向父进程区域→需要全局重定位；(C2) 需要强 μprocess 间隔离 + user/kernel 隔离→但不能牺牲 SASOS 的 lightweightness。
2. **"CHERI 能力——一个硬件机制同时解决两个正交挑战"**：CHERI 将 64-bit pointer 扩展为 128-bit capability（含 bounds、permissions、unforgeable validity tag）。对 C1：tagged memory 精确标识哪些位置包含指针→只重定位真正的指针（数据字不被误改）。对 C2：硬件 enforced capability checks 提供 intra-address-space 隔离→不需要页表切换或 VM 边界。类似 InfiniDefrag "GPA 是虚拟的"——利用已有硬件能力解决不同层次的问题。
3. **"CoPA (Copy-on-Pointer-Access) = fork 优化的新维度——不是 CoW"**：传统 CoW 在首次 write 时复制页→CoPA 在首次 **load pointer** 时复制页。子进程读取数据不需要复制→只有真正需要 traverse 指针时才触发重定位。CHERI 的 capability-load fault 机制使 CoPA 成为可能（硬件精确 trap 指针访问→非 CHERI 硬件无法实现）。CoPA 比同步复制减少 fork 延迟 **89×**。类似 VTC "index mapping 替代 data copy"——懒惰到实际使用的最后一刻。
4. **"构建于 Unikraft 之上——单一内核 + SAS → 极简 OS"**：利用 Unikraft 的 library OS 架构→应用和 kernel 编译为同一二进制→消除 syscall/user 模式切换→54μs fork（vs 传统 ~200μs）。

- 来源：μFork(SOSP'25)

### 实践启发
- **"CHERI 不仅是安全检查——也是 OS 构建块"**：μFork 将 capability 用于指针重定位、进程隔离、lazy relocation——将 CHERI 从 "memory safety tool" 提升为 "OS design primitive"。类似 MPK 基于硬件保护键实现 extra-isolated 沙箱——硬件能力可以被重新解释为 OS 机制
- **"CoPA 揭示 fork 优化的新维度——pointer-aware lazy evaluation"**：传统 CoW 只区分 read vs write→CoPA 进一步区分 data access vs pointer access→在更细粒度上应用 lazy evaluation。Pointers 是特殊数据——不应在仅读数据时迫使其复制
- **"SASOS 的 lightweightness 在 fork-intensive 场景中价值最大"**：FaaS worker warm-up +24% 吞吐、Redis snapshot 1.4-1.9× 加速、54μs fork——轻量 fork 使 SASOS 在快速快照、高密度多 worker、FaaS cold start 等场景胜出。非 CHERI 硬件上的等价实现（如 pointer masking 或 compiler-assisted relocation）是值得探索的方向
- **"POSIX 兼容性不是可选项——它是 OS 研究的 hard constraint"**：μFork 能运行 unmodified Redis/Nginx→SASOS 的 adoption barrier 降至零。类似 hS/Incr/LithOS 的 "bolt-on transparency" 哲学——研究系统必须兼容已有生态
